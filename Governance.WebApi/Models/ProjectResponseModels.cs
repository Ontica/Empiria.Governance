...
//          resourceTypeId = process.ResourceType.Id,
    ...
    static internal ICollection ToResponse(this IList<ProcessActivity> list) {
      ...
      foreach (var activity in list) {
        ...
        if (activity.ProcedureId != -1) {
          procedure = Procedure.Parse(activity.ProcedureId);
        } else {
          procedure = Procedure.Empty;
        }
        var involvedParty = procedure.Authority.Entity;

        var item = new {
          uid = activity.UID,
          taskType = activity.TaskType,
          involvedParty = involvedParty.IsEmptyInstance
                                        ? String.Empty : involvedParty.Alias,
